...
function parseEom(text) {
  ...
  for (const blk of blocks) {
    ...
    // Find actual time pairs (A-prefixed)
    const actualPairs = [...rest.matchAll(/A(\d{1,2}:\d{2})\s*[-–]\s*A(\d{1,2}:\d{2})/g)];
    let actIdx = 0;

    // Find ALL time pairs (scheduled or actual) — captures bare HH:MM - HH:MM too
    const allTimePairs = [...rest.matchAll(/A?(\d{1,2}:\d{2})\s*[-–]\s*A?(\d{1,2}:\d{2})/g)];
    let allIdx = 0;
    ...
    for (let i = 0; i < routeMatches.length; i++) {
      ...
      let std_local = null, sta_local = null;
      if (allTimePairs[allIdx]) {
        std_local = hhmm(allTimePairs[allIdx][1]);
        sta_local = hhmm(allTimePairs[allIdx][2]);
        allIdx++;
      }
      ...
      sectors.push({
        ...
        atd_local,
        ata_local,
        std_local: std_local || atd_local,
        sta_local: sta_local || ata_local,
      });
    }
    ...
    for (const row of rows) {
      ...
      if (finalActual) {
        ...
        if (am) {
          ...
          // Always capture the time as scheduled (both A-prefixed and bare
          // times represent what actually happened; the "Actual times" column
          // header covers both). For STD/STA display, the raw time is useful
          // regardless of the A-prefix.
          std_local = t1;
          sta_local = t2;
          atdMin = t2m_local(t1);
          ataMin = t2m_local(t2);
...
function parseGridFromItems(page1Items, pilot, year, mo) {
  ...
  for (let colIdx = 0; colIdx < buckets.length; colIdx++) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      let atd = null, std = null, dep = null, arr = null, ata = null, sta = null, star = false;
      ...
      while (j < items.length) {
        ...
        if (atd === null && !dep && TIME_RE.test(t)) {
          ...
          const timeStr = `${hh.padStart(2, "0")}:${mm2}`;
          if (aFlag === "A") {
            atd = { time: timeStr, actual: true };
          } else {
            // Non-A-prefixed time before DEP = scheduled departure (STD).
            std = timeStr;
          }
          j++; continue;
        }
        ...
        if (arr && ata === null && TIME_RE.test(t)) {
          ...
          const timeStr = `${hh.padStart(2, "0")}:${mm2}`;
          if (aFlag === "A") {
            ata = { time: timeStr, actual: true };
          } else {
            // Non-A-prefixed time after ARR = scheduled arrival (STA).
            sta = timeStr;
            ata = { time: timeStr, actual: false };
          }
          j++; continue;
        }
      ...
      sectors.push({
        ...
        std_local: std || (atd?.actual ? atd.time : (atd ? atd.time : null)),
        sta_local: sta || (ata?.actual ? ata.time : (ata ? ata.time : null)),
        is_dhf: star,
        is_dht: false,
    ...
    if (ocSectors.length) {
      ...
      for (const oc of ocSectors) {
        ...
        sectors.push({
          ...
          std_local: gs?.std_local || gs?.atd_local || null,
          sta_local: gs?.sta_local || gs?.ata_local || null,
          _dateFromOC: true,
        });
      }
      for (const [fltNo, gridList] of gridByFlt) {
        ...
        for (let k = used; k < gridList.length; k++) {
          ...
          sectors.push({
            ...
            std_local: gs.std_local || gs.atd_local,
            sta_local: gs.sta_local || gs.ata_local,
          });
        }
      }
