...
export async function checkProfileExists(email, staffId) {
  ...
  try {
    if (email) {
      const { data } = await supabaseServer.from('profiles').select('*').eq('email', email.trim().toLowerCase()).maybeSingle();
      if (data) return { exists: true, field: 'Email', profile: data };
    }
    if (staffId) {
      const { data } = await supabaseServer.from('profiles').select('*').eq('staff_id', staffId.trim()).maybeSingle();
      if (data) return { exists: true, field: 'Staff ID', profile: data };
    }
  } catch (e) {}

  if (email && inMemoryStore.profile?.email === email) {
    return { exists: true, field: 'Email', profile: inMemoryStore.profile };
  }
  if (staffId && inMemoryStore.profile?.staff_id === staffId) {
...
export async function addScheduleToStore(pdfBuffer, fileName, currentUserProfile = null) {
  ...
  const rawStaffId = parsed.metadata.staff_id || currentUserProfile?.staff_id || inMemoryStore.profile.staff_id;
  const rawFullName = parsed.metadata.full_name || currentUserProfile?.full_name || inMemoryStore.profile.full_name;
  ...
  const profileInput = {
    ...
    base_airport: base || currentUserProfile?.base_airport || inMemoryStore.profile.base_airport,
    role: role || currentUserProfile?.role || inMemoryStore.profile.role,
    aircraft_type: fleet || currentUserProfile?.aircraft_type || inMemoryStore.profile.aircraft_type
  };
